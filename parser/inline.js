...
const MODE_INLINE_UNDERLINE = 11;
const MODE_INLINE_MATH = 12;
	...
	for (let i = 0; i < text.length; ++i) {
		...
		switch (char) {
			...
			case "~":
				...
				continue;
			case "-":
				if (mode === MODE_INLINE_UNDERLINE) {
					ast.push(new nodes.Underline(stack));
					mode = MODE_DEFAULT;
				} else {
					if (!helper.isEmpty(stack)) {
						ast.push(new nodes.Text(stack));
					}
					mode = MODE_INLINE_UNDERLINE;
				}
				stack = "";
				continue;
			case "`":
				...
				continue;
			case "$":
				if (mode === MODE_INLINE_MATH) {
					ast.push(new nodes.InlineMath(stack));
					mode = MODE_DEFAULT;
				} else {
					if (!helper.isEmpty(stack)) {
						ast.push(new nodes.Text(stack));
					}
					mode = MODE_INLINE_MATH;
				}
				stack = "";
				continue;
			case "<":
